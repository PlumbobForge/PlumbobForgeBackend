...
public class MathUtils
{
	...
	public const float kOutOfWorld = -20000f;

	public const float kInvalid = float.PositiveInfinity;

	private static readonly long[] kPrimeNumbers = new long[168]
	{
	...
	public static bool IsSimilar(float x, float y)
	{
		return Math.Abs(x - y) < 1E-05f;
	}

	public static bool IsSimilar(float x, float y, float tolerance)
	{
	...
	public static List<Vector2> FactorsOf(int value)
	{
		...
		{
			long result;
			long num2 = Math.DivRem(a, i, out result);
			if (result == 0)
			{
				list.Add(new Vector2(i, num2));
			}
		}
		return list;
	}
	...
	public static List<long> PrimeFactorsOf(long longValue)
	{
		...
		{
			int i;
			for (i = 0; i < kPrimeNumbers.Length; i++)
			{
				long num = kPrimeNumbers[i];
				long result;
				long num2 = Math.DivRem(longValue, num, out result);
				if (result == 0)
				{
					list.Add(num);
					longValue = num2;
	...
	{
		bool result = true;
		List<long> list = PrimeFactorsOf(0L);
		if (list.Count != 0)
		{
			result = false;
		}
		list = PrimeFactorsOf(1L);
		if (list.Count != 0)
		{
			result = false;
		}
		list = PrimeFactorsOf(2L);
		if (list.Count != 1 || list[0] != 2)
		{
			result = false;
		}
		list = PrimeFactorsOf(6L);
		if (list.Count != 2 || list[0] != 2 || list[1] != 3)
		{
			result = false;
		}
		list = PrimeFactorsOf(24L);
		if (list.Count != 4 || list[0] != 2 || list[1] != 2 || list[2] != 2 || list[3] != 3)
		{
			result = false;
		}
		list = PrimeFactorsOf(100L);
		if (list.Count != 4 || list[0] != 2 || list[1] != 2 || list[2] != 5 || list[3] != 5)
		{
			result = false;
		}
		return result;
